...
public class SmtpConnectionController implements SmtpConnectionApi {
    ...
    public ResponseEntity<List<SmtpConnectionDto>> listConnections() {
        ...
        return ResponseEntity.ok(smtpConnectionService.getConnectionsForUser(user.getUuid()));
    }
    ...
    public ResponseEntity<SmtpConnectionDto> getConnection(String id) {
        UserEntity user = securityContextService.requireUser();
        return ResponseEntity.ok(smtpConnectionService.getOwnedConnectionDto(id, user.getUuid()));
    }
    ...
    public ResponseEntity<SmtpConnectionDto> updateConnection(
        ...
        UserEntity user = securityContextService.requireUser();
        return ResponseEntity.ok(smtpConnectionService.updateConnection(id, user.getUuid(), dto));
    }

    @Override
    public ResponseEntity<Void> deleteConnection(String id) {
        UserEntity user = securityContextService.requireUser();
        smtpConnectionService.deleteConnection(id, user.getUuid());
        return ResponseEntity.noContent().build();
    }

    @Override
    public ResponseEntity<Page<MailDto>> getMails(String id, int page, int size) {
        UserEntity user = securityContextService.requireUser();
        return ResponseEntity.ok(mailService.getMailsForConnection(id, user.getUuid(), page, size));
    }
}
